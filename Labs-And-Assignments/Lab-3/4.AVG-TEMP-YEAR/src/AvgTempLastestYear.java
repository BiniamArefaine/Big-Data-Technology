...
public class AvgTempLastestYear extends Configured implements Tool
{


	public static class AverageTempMapper extends Mapper<LongWritable, Text,SortKeyYear , Pair>
	{
		...
		public void setup(Context context){
			...
			public void map(LongWritable key,Text value, Context context){
				...
				new Pair(p.getTemp()+temp.getTemp(),p.getCount()+1));
				}
				else
			...
			public void cleanup(Context context) throws IOException, InterruptedException {
				...
				for(String key: inMapperCombiner.keySet()){
					...
					context.write(new SortKeyYear(key), inMapperCombiner.get(key));
				}
			}
	}
	
	public static class AverageTempReducer extends Reducer<SortKeyYear, Pair, Text, DoubleWritable>
	{
		
		@Override
		public void reduce(SortKeyYear key, Iterable<Pair> values, Context context) throws IOException, InterruptedException
		{
			int count=0;
			double sum=0;
			for(Pair val:values){
	...
	public static void main(String[] args) throws Exception
	{
		...
		int res = ToolRunner.run(conf, new AvgTempLastestYear(), args);
